...
class ListView {
        ...
        foreach ($this->dtp->CTags as $tagid => $ctag) {
            if ($ctag->GetName() == "list") {
                ...
                $this->dtp->Assign($tagid, $this->GetArcList(
                                $limitstart, $row, $ctag->GetAtt("col"), $ctag->GetAtt("titlelen"), $ctag->GetAtt("infolen"), $ctag->GetAtt("imgwidth"), $ctag->GetAtt("imgheight"), $ctag->GetAtt("listtype"), $ctag->GetAtt("orderby"), $InnerText, $ctag->GetAtt("tablewidth"), $ismake, $ctag->GetAtt("orderway")
                        )
                );
    ...
    function GetPageListST($list_len, $listitem = "index,end,pre,next,pageno") {
        ...
        //获得上一页和主页的链接
        if ($this->PageNo != 1) {
            $indexpage = "<a href='" . str_replace("{page}", 1, $tnamerule) . "' style='border-radius: 8px;'>首页</a>\r\n";
            $prepage.="<a href='" . str_replace("{page}", $prepagenum, $tnamerule) . "' class='prev disable'>&lt;</a>\r\n";
            $endpage = "<a href='" . str_replace("{page}", $totalpage, $tnamerule) . "'>尾页</a>\r\n";
        } else {
            $indexpage = "<a href='" . str_replace("{page}", 1, $tnamerule) . "' style='border-radius: 8px;'>首页</a>\r\n";
            $prepage.="<a href='javascript:void(0);' class='prev disable'>&lt;</a>\r\n";
            $endpage = "<a href='" . str_replace("{page}", $totalpage, $tnamerule) . "' style='border-radius: 8px;'>尾页</a>\r\n";
        }

        //下一页,未页的链接
        if ($this->PageNo != $totalpage && $totalpage > 1) {
            $indexpage = "<a href='" . str_replace("{page}", 1, $tnamerule) . "' style='border-radius: 8px;'>首页</a>\r\n";
            $nextpage.="<a href='" . str_replace("{page}", $nextpagenum, $tnamerule) . "' class='next'>&gt;</a>\r\n";
            $endpage = "<a href='" . str_replace("{page}", $totalpage, $tnamerule) . "' style='border-radius: 8px;'>尾页</a>\r\n";
        } else {
            $indexpage = "<a href='" . str_replace("{page}", 1, $tnamerule) . "' style='border-radius: 8px;'>首页</a>\r\n";
            $nextpage.="<a href='javascript:void(0);' class='next'>&gt;</a>\r\n";
            $endpage = "<a href='" . str_replace("{page}", $totalpage, $tnamerule) . "'style='border-radius: 8px;'>尾页</a>\r\n";
        }
        ...
        if ($totalpage >= 6) {//这个是在分页数量大于的情况下执行下面的语句
            $totallist = 5 + $this->PageNo; //这里是要显示几页，如果是10页就写9
        ...
        for ($j = $d; $j <= $totallist; $j++) {
            if ($j == $this->PageNo) {
                $listdd.= "<a href='javascript:void(0);' class='current'>$j</a>";
            } else {
    ...
    function GetPageListDM($list_len, $listitem = "index,end,pre,next,pageno") {
        ...
        if ($this->PageNo != 1) {
            ...
            $indexpage = "<a href='" . $purl . "PageNo=1'>1</a>\r\n";
        } else {
            $indexpage = "<a>1</a>\r\n";
        }
        if ($this->PageNo != $totalpage && $totalpage > 1) {
            $nextpage.="<a href='" . $purl . "PageNo=$nextpagenum'>&gt;</a>\r\n";
            $endpage = "<a href='" . $purl . "PageNo=$totalpage'>..." . $totalpage . "</a>\r\n";
        } else {
            $endpage = "<a>$totalpage</a>\r\n";
        }
