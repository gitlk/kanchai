...
if(isset($_GET['ajax'])){
    ...
    $flag = isset($_GET['flag']) ? intval($_GET['flag']): '';//页码
    ...
    $typesql = $typeid ? " WHERE typeid=$typeid and flag FIND_IN_SET($flag)" : '';//这个是用于首页实现瀑布流加载，因为首页加载数据是无需分类的，所以要加以判断，如果无需
    ...
    FROM `lk_archives` as a JOIN `lk_arctype` AS t ON a.typeid=t.id $typesql ORDER BY id DESC LIMIT $start,$pagesize";
    $dsql->SetQuery($sql);
    $dsql->Execute('list');
